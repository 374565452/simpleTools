...
import com.simple.tools.model.ArrayType;
import com.simple.tools.model.Json2JavaElement;
...
public class JsonUtils {
	...
	public static void main(String[] args) {
		/// 读取json字符串
		/*
		 * String json = FileUtils.readToString(new File( "temp" +
		 * File.separator + "Json" + File.separator + "JsonString.txt"),
		 * "UTF-8");
		 */
	...
	public static void parseJson2Java(String jsonStr) {
		...
		String javaBeanStr = createJavaBean("com.my", "Test", jsonBeanTree);

		System.out.println(javaBeanStr);
		// 将生成的内容写入到文件中去
		/*
		 * FileUtils.writeString2File(javaBeanStr, new File( "temp" +
		 * File.separator + "Json" + File.separator + "JsonBean.java"));
		 */
		/*
		 * FileUtils.writeString2File(javaBeanStr, new File(
		 * "gen"+File.separator+"JsonBean.java"));
		 */
	}
	 ...
	 */
	public static String getJavaFromJson(String packageName, String className, String jsonStr) {
		// 解析获取整个json结构集合
		List<Json2JavaElement> jsonBeanTree = getJsonBeanTree(jsonStr);

		// 利用获取到的json结构集合,创建对应的javabean文件内容
		String javaBeanStr = createJavaBean(packageName, className, jsonBeanTree);
	 ...
	 */
	public static String createJavaBean(String packageName, String className, List<Json2JavaElement> jsonBeanTree) {
		StringBuilder sb = new StringBuilder();
		StringBuilder sbGetterAndSetter = new StringBuilder();
		if (packageName == null || packageName.equals("")) {
			packageName = "com.parse.jsonbean";
		}
